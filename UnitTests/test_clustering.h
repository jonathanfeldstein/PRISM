...
TestCount test_compute_top_paths(vector<NodeRandomWalkData> sample_path_data, size_t max_num_paths, size_t path_length, MatrixXd expected_top_paths);
TestCount test_two_means(MatrixXd all_points, vector<size_t> expected_cluster_labels_after_one_iteration, vector<size_t> expected_after_two_iterations);
TestCount test_hierarchical_two_means(MatrixXd npc, MatrixXd nfv, const vector<size_t>& expected_cluster_labels);
bool test_cluster_nodes_by_truncated_hitting_times(const vector<NodeRandomWalkData>& nodes_of_type,double threshold_hitting_time_difference,vector<size_t> expected_clustering);
