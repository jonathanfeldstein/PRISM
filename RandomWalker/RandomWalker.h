...
private:
    ...
    pair<map<string,NodeRandomWalkData>, size_t> run_random_walks(string const& source_node);
    void update_node_data_with_random_walk(string const& source_node, map<string, NodeRandomWalkData> &nodes_random_walk_data);
    size_t compute_number_of_additional_walks(map<string, NodeRandomWalkData> &nodes_random_walk_data, size_t number_of_completed_walks);
    size_t compute_number_of_unique_paths(map<string, NodeRandomWalkData> &nodes_random_walk_data);

public:
    RandomWalker(HyperGraph hypergraph, RandomWalkerConfig config);
    ~RandomWalker();

    map<string, NodeRandomWalkData> generate_node_random_walk_data(string const& source_node);
