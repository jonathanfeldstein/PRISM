...
pair<set<size_t>, vector<set<size_t>>> cluster_nodes_by_path_similarity(vector<NodeRandomWalkData> nodes,
    ...
    pair<set<size_t>, vector<vector<NodeRandomWalkData>>> distance_symmetric_clusters = cluster_nodes_by_truncated_hitting_times(nodes, theta_sym);
...
pair<set<size_t>, vector<vector<NodeRandomWalkData>>> cluster_nodes_by_truncated_hitting_times(vector<NodeRandomWalkData> nodes,
    ...
    sort(nodes.begin(), nodes.end()); // TODO check order correct
    double current_hitting_time = nodes[0].get_average_hitting_time();
